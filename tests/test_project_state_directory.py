...
def test_the_state_directory_is_dot_claude_inside_the_project(tmp_path) -> None:
    state_directory = ProjectStateDirectory(str(tmp_path))
    assert state_directory.directory_path == os.path.join(str(tmp_path), ".claude")


def test_a_named_document_lives_inside_the_state_directory(tmp_path) -> None:
    state_directory = ProjectStateDirectory(str(tmp_path))
    document = state_directory.json_document("thing.json")
    assert document.file_path == os.path.join(str(tmp_path), ".claude", "thing.json")
...
def test_a_named_subdirectory_path_is_offered_without_being_created(tmp_path) -> None:
    ...
    state_directory = ProjectStateDirectory(str(tmp_path))
    subdirectory_path = state_directory.subdirectory_path("history")
    assert subdirectory_path.endswith(os.path.join(".claude", "history"))
...
def test_reading_a_document_that_was_never_written_gives_the_default(tmp_path) -> None:
    document = ProjectStateDirectory(str(tmp_path)).json_document("absent.json")
...
def test_a_written_document_reads_back(tmp_path) -> None:
    document = ProjectStateDirectory(str(tmp_path)).json_document("thing.json")
    document.write_dictionary({"answer": 42})
    assert document.read_dictionary_or_default({}) == {"answer": 42}


def test_writing_creates_the_state_directory(tmp_path) -> None:
    document = ProjectStateDirectory(str(tmp_path)).json_document("thing.json")
    document.write_dictionary({"answer": 42})
...
def test_exists_reports_only_real_content(tmp_path) -> None:
    document = ProjectStateDirectory(str(tmp_path)).json_document("thing.json")
